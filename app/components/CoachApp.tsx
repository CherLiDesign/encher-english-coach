...
function speakVocabularyTerm(term: string) {
...
export function CoachApp({ user, onSignOut }: { user: User; onSignOut: () => Promise<void> }) {
  ...
  const [librarySelectedTerm, setLibrarySelectedTerm] = useState("");
  ...
  const lastSavedItem = useMemo(() => vocabulary.find((item) => sameVocabularyTerm(item.term, lastSavedTerm)) ?? null, [lastSavedTerm, vocabulary]);
  ...
  useEffect(() => {
    ...
    supabase.from("vocabulary_items").select("item_data").eq("user_id", user.id).order("updated_at", { ascending: false }).then(async ({ data, error }) => {
      ...
      const pending = loaded.filter((item) => item.enrichmentStatus === "pending" || item.enrichmentStatus === "unavailable" || item.definition.toLowerCase().includes("looking up"));
  ...
  const openVocabulary = (term = "") => { setLibrarySelectedTerm(term); navigate("vocabulary"); };
  ...
  const addQuickWord = async () => {
    ...
    if (existing) {
      ...
      flash(`“${existing.term}” is already in your account · showing its meaning`);
      return;
    }
  ...
  const nextCandidate = () => {
    ...
    else { openVocabulary(currentCandidate.term); flash("Candidate check complete"); }
  };
  ...
  return <div className="app-shell app-v2">
    ...
    {view === "vocabulary" && <VocabularyPage items={vocabulary} selectedTerm={librarySelectedTerm} navigate={navigate} />}
...
function AppHeader({ activeTab, cloudState, navigate }: { activeTab: string; cloudState: string; navigate: (view: View) => void }) {
  const titles: Record<string, string> = { home: "Home", practice: "Practice", me: "Me" };
...
function QuickLearnCard({ item, openVocabulary }: { item: VocabularyItem; openVocabulary: (term?: string) => void }) {
  const pending = item.enrichmentStatus === "pending";
  const unavailable = item.enrichmentStatus === "unavailable";
  return <section className={`quick-learn-card ${pending || unavailable ? "waiting" : "ready"}`} aria-live="polite">
    <div className="quick-learn-heading"><div><span className="section-kicker">{pending ? "FINDING THE RIGHT MEANING" : unavailable ? "SAVED · NEEDS MORE CONTEXT" : "JUST ADDED · STUDY MODE"}</span><h2>{item.term}</h2>{!pending && !unavailable && <p><span>{item.partOfSpeech}</span> {item.pronunciation}</p>}</div>{!pending && !unavailable && <button type="button" className="hear-word" onClick={() => speakVocabularyTerm(item.term)} aria-label={`Hear ${item.term} pronounced`}>▶ <span>Hear it</span></button>}</div>
    {pending ? <div className="quick-meaning-loading"><span className="memory-spinner" /><div><b>Your word is safely saved.</b><p>Now adding its meaning, pronunciation, usage, and a workplace example…</p></div></div> : unavailable ? <div className="quick-meaning-loading unavailable"><Icon name="spark" /><div><b>We could not verify this meaning without context.</b><p>The word remains in your library. Add the meeting sentence later so Encher can teach the exact meaning you heard.</p></div></div> : <>
      <div className="quick-meaning"><small>MEANING</small><p>{item.definition}</p>{item.chinese && <span>{item.chinese}</span>}</div>
      <div className="quick-learning-grid"><article><small>HOW TO USE IT</small><p>{item.usageNote}</p>{item.collocations?.length ? <span><b>Common:</b> {item.collocations.join(" · ")}</span> : null}</article><article><small>WORKPLACE EXAMPLE</small><blockquote>“{item.newExample}”</blockquote></article></div>
    </>}
    <div className="quick-learn-actions"><span><Icon name="check" /> Saved in your private Word Library</span><button type="button" onClick={() => openVocabulary(item.term)}>Open full entry <Icon name="arrow" /></button></div>
  </section>;
}

function HomePage({ quickWord, setQuickWord, addQuickWord, quickSaveState, quickSaveError, memoryLoadState, memoryVerifiedAt, accountEmail, personalWordCount, verifyAccountMemory, lastSavedItem, recentQuickWords, startImport, openVocabulary }: { quickWord: string; setQuickWord: (value: string) => void; addQuickWord: () => Promise<void>; quickSaveState: QuickSaveState; quickSaveError: string; memoryLoadState: MemoryLoadState; memoryVerifiedAt: string | null; accountEmail: string; personalWordCount: number; verifyAccountMemory: () => void; lastSavedItem: VocabularyItem | null; recentQuickWords: VocabularyItem[]; startImport: (purpose: ImportPurpose) => void; openVocabulary: (term?: string) => void }) {
  ...
  return <main className="app-page add-page">
    <section className="tab-intro"><span className="eyebrow">YOUR WORKPLACE ENGLISH MEMORY</span><h1>Add a word.</h1><p>Capture it in seconds. Encher will save it to your account and turn it into practice.</p></section>
    <section className="capture-hub add-word-card">
      ...
      {lastSavedItem && quickSaveState !== "error" && <div className="memory-save-status saved" role="status"><Icon name="check" /><div><b>Saved and read back — meaning shown below</b><p>“{lastSavedItem.term}” was written to {accountEmail}, then independently read back.</p></div><button type="button" onClick={() => openVocabulary(lastSavedItem.term)}>Full entry</button></div>}
      <p>“Saved” means Encher wrote the word, then independently read the same word back from your private account.</p>
    </section>
    {lastSavedItem && quickSaveState !== "error" && <QuickLearnCard item={lastSavedItem} openVocabulary={openVocabulary} />}
    ...
    <section className="recent-memory">
      <div className="section-heading library-heading"><div><span className="section-kicker">ANSWERS ALWAYS VISIBLE</span><h2>Your Word Library</h2><p>Review every meaning, pronunciation, usage pattern, and example whenever you want.</p></div><button type="button" onClick={() => openVocabulary()}>Open all {personalWordCount} <Icon name="arrow" /></button></div>
      {recentQuickWords.length ? <div>{recentQuickWords.map((item) => <button key={item.id} type="button" onClick={() => openVocabulary(item.term)}><span><b>{item.term}</b><small>{item.enrichmentStatus === "pending" ? "Saved · adding meaning…" : item.definition}</small></span><em><Icon name="check" /> Saved</em><Icon name="arrow" /></button>)}</div> : <div className="recent-memory-empty"><Icon name="book" /><span><b>No quick-added words yet</b><small>Your confirmed saves will appear here immediately.</small></span></div>}
    </section>
...
function PracticeHub({ lane, setLane, navigate, openVocabulary, dueCount, analyzed }: { lane: PracticeLane; setLane: (lane: PracticeLane) => void; navigate: (view: View) => void; openVocabulary: (term?: string) => void; dueCount: number; analyzed: boolean }) {
  ...
  return <main className="app-page practice-hub"><section className="practice-intro"><span className="eyebrow">PRACTICE FROM YOUR REAL WORK</span><h1>Practice</h1><p>Choose an outcome, not a school subject. Grammar and pronunciation appear only when they help the real communication problem.</p></section><div className="practice-tabs" role="tablist" aria-label="Practice type">{tabs.map((tab) => <button key={tab.id} role="tab" aria-selected={lane === tab.id} className={lane === tab.id ? "active" : ""} onClick={() => setLane(tab.id)}>{tab.label}</button>)}</div><section className="practice-lane" role="tabpanel">
    ...
    {lane === "vocabulary" && <><section className="priority-practice listen-practice"><div><span className="section-kicker">PRACTICE MODE · ANSWERS HIDDEN</span><h2>{dueCount} items selected for you</h2><p>Recall first; after every answer, see the correct meaning, usage, example, and next review date.</p><div className="practice-tags"><span>Due today</span><span>From your meetings</span><span>Adaptive review</span></div></div><button onClick={() => navigate("today")}>Start vocabulary practice <Icon name="arrow" /></button></section><section className="practice-support"><button onClick={() => openVocabulary()}><span><Icon name="book" /></span><div><b>Open Word Library</b><small>Study every meaning, pronunciation, usage pattern, and example — answers visible</small></div><Icon name="arrow" /></button></section></>}
...
function VocabularyPage({ items, selectedTerm, navigate }: { items: VocabularyItem[]; selectedTerm: string; navigate: (view: View) => void }) {
  const uniqueItems = useMemo(() => dedupeVocabularyItems(items), [items]);
  const requestedItem = useMemo(() => uniqueItems.find((item) => selectedTerm && sameVocabularyTerm(item.term, selectedTerm)), [selectedTerm, uniqueItems]);
  const [selectedId, setSelectedId] = useState(requestedItem?.id ?? uniqueItems[0]?.id ?? "");
  const [libraryQuery, setLibraryQuery] = useState("");
  const filteredItems = useMemo(() => {
    const query = vocabularyTermKey(libraryQuery);
    return query ? uniqueItems.filter((item) => `${item.term} ${item.definition} ${item.chinese ?? ""}`.toLocaleLowerCase("en-US").includes(query)) : uniqueItems;
  }, [libraryQuery, uniqueItems]);
  ...
  if (!selected) return <main className="app-page detail-page wide"><PageHead title="Word Library" subtitle="Study mode · meanings and examples are always visible here." onBack={() => navigate("practice")} /><section className="vocabulary-empty"><Icon name="book" /><h2>No personal words saved yet.</h2><p>Example words are never shown as your memory. Add one real word and Encher will verify it against your private account before it appears here.</p><button className="solid-button" onClick={() => navigate("home")}>Add my first word</button></section></main>;
  return <main className="app-page detail-page wide"><PageHead title="Word Library" subtitle="Study mode · meanings, pronunciation, usage, and examples are always visible." onBack={() => navigate("practice")} /><section className="study-mode-banner"><Icon name="book" /><div><b>Study mode · answers visible</b><p>Use this library to learn or look something up. Practice mode hides the answer until you respond.</p></div><button type="button" onClick={() => navigate("today")}>Practice with answers hidden</button></section><div className="library-search"><input type="search" value={libraryQuery} onChange={(event) => setLibraryQuery(event.target.value)} placeholder="Search your words and phrases" aria-label="Search your Word Library" /><span>{filteredItems.length} of {uniqueItems.length}</span></div><div className="library-layout"><section className="vocab-list"><div className="list-toolbar"><span>{uniqueItems.length} personal items</span><button onClick={() => navigate("today")}>Review due</button></div>{filteredItems.length ? filteredItems.map((item) => <button key={item.id} className={selected?.id === item.id ? "selected" : ""} onClick={() => setSelectedId(item.id)}><span><b>{item.term}</b><small>{item.enrichmentStatus === "pending" ? "Adding meaning…" : item.definition}</small></span><em>{item.enrichmentStatus === "pending" ? "Adding meaning…" : item.status}</em></button>) : <p className="library-no-results">No saved words match “{libraryQuery}”.</p>}</section>{selected && <section className="vocab-detail"><div className="detail-top"><div><div className="vocab-tags"><span className="status-pill">{selected.status}</span>{selected.tags?.map((tag) => <span key={tag} className="source-tag">{tag}</span>)}</div><h2>{selected.term}</h2><p className="pronunciation">{selected.pronunciation} {selected.partOfSpeech && <em>{selected.partOfSpeech}</em>} <button type="button" aria-label={`Hear ${selected.term} pronounced`} onClick={() => speakVocabularyTerm(selected.term)}>▶</button></p></div><ProgressRing value={Math.round(Object.values(selected.mastery).reduce((a, b) => a + b, 0) / 6)} /></div><div className="library-meaning"><small>MEANING</small><p className="definition">{selected.definition}</p>{selected.chinese && <p className="chinese">{selected.chinese}</p>}</div>{selected.usageNote && <div className="dictionary-usage"><b>How to use it</b><p>{selected.usageNote}</p>{selected.collocations?.length ? <small>Common patterns: {selected.collocations.join(" · ")}</small> : null}</div>}<div className="library-example"><small>WORKPLACE EXAMPLE</small><blockquote>“{selected.newExample}”</blockquote>{!selected.hasOriginalContext && <p>Example supplied because the original work sentence was not captured.</p>}</div>{selected.hasOriginalContext && <div className="origin"><small>WHERE YOU FOUND IT · {selected.source}</small><blockquote>“{selected.sentence}”</blockquote><p>{selected.context}</p></div>}<div className="review-plan-card"><div><Icon name="clock" /><span><small>NEXT REVIEW</small><b>{reviewLabel}</b></span></div><div><span><small>CURRENT INTERVAL</small><b>{selected.review?.intervalDays ? `${selected.review.intervalDays} days` : "New"}</b></span><span><small>LAST RESULT</small><b>{selected.review?.lastResult?.replace("partial", "Partly correct") ?? "Not reviewed"}</b></span></div><p>Correct answers increase the interval. Incorrect answers return in the same session and again tomorrow.</p></div><h3>Mastery profile</h3><div className="mastery-grid">{Object.entries(selected.mastery).map(([key, value]) => <div key={key}><span><b>{key.replace(/([A-Z])/g, " $1")}</b><em>{value}%</em></span><i><span style={{ width: `${value}%` }} /></i></div>)}</div><div className="coach-note"><Icon name="spark" /><p><b>Why this is here</b>{selected.reason}</p></div></section>}</div></main>;
}
