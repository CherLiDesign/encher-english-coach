...
function tabFor(view: View) {
  if (["listening", "today", "sentence", "import", "review", "vocabulary"].includes(view)) return "listening";
  if (["speaking", "speaking-practice"].includes(view)) return "speaking";
...
export function CoachApp({ user, onSignOut }: { user: User; onSignOut: () => Promise<void> }) {
  ...
  const navigate = (next: View) => { setView(next); setCaptureOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const addQuickWord = () => {
    ...
    setQuickWord(""); setCaptureOpen(false); flash(`“${term}” saved`);
  };
  ...
  const runAnalysis = async () => {
    ...
    if (importPurpose === "speaking") {
      ...
      setSpeakingAnalyzed(true); setLoading(false); navigate("speaking"); flash("Your communication diagnosis is ready");
      return;
    }
  ...
  return <div className="app-shell app-v2">
    ...
    {view === "home" && <Home navigate={navigate} startImport={startImport} dueCount={dueItems.length} newLearner={vocabulary.length <= seedVocabulary.length} />}
    {view === "listening" && <ListeningHome quickWord={quickWord} setQuickWord={setQuickWord} addQuickWord={addQuickWord} startImport={startImport} navigate={navigate} dueCount={dueItems.length} />}
    {view === "speaking" && <SpeakingHome navigate={navigate} startImport={startImport} analyzed={speakingAnalyzed} />}
    ...
    <button className="capture-fab" aria-label="Quick add" onClick={() => setCaptureOpen(true)}><Icon name="plus" /></button>
    <BottomNavigation activeTab={activeTab} navigate={navigate} />
    {captureOpen && <QuickCapture quickWord={quickWord} setQuickWord={setQuickWord} addQuickWord={addQuickWord} startImport={startImport} close={() => setCaptureOpen(false)} />}
...
function AppHeader({ activeTab, cloudState, navigate }: { activeTab: string; cloudState: string; navigate: (view: View) => void }) {
  const titles: Record<string, string> = { home: "Today", listening: "Listen", speaking: "Speak", me: "Me" };
...
function BottomNavigation({ activeTab, navigate }: { activeTab: string; navigate: (view: View) => void }) {
  const tabs: { id: string; label: string; icon: IconName; view: View }[] = [
    ...
    { id: "listening", label: "Listen", icon: "listen", view: "listening" },
    { id: "speaking", label: "Speak", icon: "speak", view: "speaking" },
...
function QuickCapture({ quickWord, setQuickWord, addQuickWord, startImport, close }: { quickWord: string; setQuickWord: (value: string) => void; addQuickWord: () => void; startImport: (purpose: ImportPurpose) => void; close: () => void }) {
  return <div className="capture-overlay" role="presentation" onKeyDown={(event) => event.key === "Escape" && close()} onMouseDown={(event) => event.target === event.currentTarget && close()}><section className="capture-sheet" role="dialog" aria-modal="true" aria-labelledby="capture-title"><div className="sheet-handle" /><div className="sheet-title"><div><span className="section-kicker">CAPTURE FROM WORK</span><h2 id="capture-title">Add it before you forget it.</h2></div><button aria-label="Close quick add" onClick={close}>×</button></div><label className="capture-word">Word or phrase<div><input aria-label="Word or phrase" value={quickWord} onChange={(event) => setQuickWord(event.target.value)} onKeyDown={(event) => event.key === "Enter" && addQuickWord()} placeholder="e.g. walk it back" /><button disabled={!quickWord.trim()} onClick={addQuickWord}>Save</button></div></label><div className="capture-options"><button onClick={() => startImport("listening")}><span><Icon name="text" /></span><div><b>Paste for listening</b><small>Find words and sentences you missed</small></div><Icon name="arrow" /></button><button onClick={() => startImport("speaking")}><span><Icon name="upload" /></span><div><b>Paste for speaking</b><small>Find clarity, grammar, and expression issues</small></div><Icon name="arrow" /></button><button className="v2-option" disabled><span><Icon name="record" /></span><div><b>Record a meeting</b><small>Audio capture and pronunciation · V2</small></div><em>SOON</em></button></div></section></div>;
}

function Home({ navigate, startImport, dueCount, newLearner }: { navigate: (view: View) => void; startImport: (purpose: ImportPurpose) => void; dueCount: number; newLearner: boolean }) {
  return <main className="app-page home-v2"><section className="home-intro"><span className="eyebrow">WEEK 1 · BUILDING YOUR FOUNDATION</span><h1>One useful improvement today.</h1><p>Your practice is selected from the communication problems with the highest impact at work.</p></section><section className="mission-card"><div className="mission-top"><span><Icon name="speak" /></span><small>BEST NEXT STEP · 10 MIN</small></div><h2>Make your recommendation clear in the first sentence.</h2><p>You explain useful context, but coworkers sometimes have to infer what you want them to do.</p><button onClick={() => navigate("speaking-practice")}>Start focused speaking <Icon name="arrow" /></button><div className="why-row"><Icon name="spark" /><span><b>Why this now</b>High communication impact · found 3 times</span></div></section>{newLearner && <section className="first-week-card"><Icon name="spark" /><div><b>Build your learner model this week</b><p>1. Add one real meeting transcript &nbsp; 2. Practice for 10 minutes &nbsp; 3. Bring in the next meeting so Encher can verify improvement.</p></div></section>}<section className="home-quick"><div className="section-heading"><h2>Bring in today’s work</h2><span>20 seconds</span></div><div><button onClick={() => startImport("listening")}><Icon name="listen" /><span><b>What I heard</b><small>Add words or a transcript</small></span></button><button onClick={() => startImport("speaking")}><Icon name="speak" /><span><b>What I said</b><small>Find expression problems</small></span></button></div></section><section className="practice-pulse"><button onClick={() => navigate("today")}><span><Icon name="listen" /></span><div><small>LISTENING READY</small><b>{dueCount} words need review</b><p>About 6 minutes</p></div><Icon name="arrow" /></button><button onClick={() => navigate("sentence")}><span><Icon name="book" /></span><div><small>SENTENCE PRACTICE</small><b>Understand the whole idea</b><p>2 meeting sentences</p></div><Icon name="arrow" /></button></section><section className="transfer-note"><Icon name="check" /><p><b>Workplace transfer detected</b>You used “trade-off” naturally in Tuesday’s product review.</p><button onClick={() => navigate("vocabulary")}>View</button></section></main>;
}

function ListeningHome({ quickWord, setQuickWord, addQuickWord, startImport, navigate, dueCount }: { quickWord: string; setQuickWord: (value: string) => void; addQuickWord: () => void; startImport: (purpose: ImportPurpose) => void; navigate: (view: View) => void; dueCount: number }) {
  return <main className="app-page"><section className="tab-intro"><span className="eyebrow">UNDERSTAND YOUR COWORKERS</span><h1>Listen</h1><p>Capture what confused you, then turn it into targeted comprehension practice.</p></section><section className="capture-hub listen-hub"><span className="section-kicker">QUICK CAPTURE</span><h2>What did you miss?</h2><div className="inline-save"><input value={quickWord} onChange={(event) => setQuickWord(event.target.value)} onKeyDown={(event) => event.key === "Enter" && addQuickWord()} aria-label="Quick add a word or phrase" placeholder="Type a word or phrase…" /><button disabled={!quickWord.trim()} onClick={addQuickWord}>Save</button></div><div className="capture-row"><button onClick={() => startImport("listening")}><Icon name="text" /><span><b>Paste meeting transcript</b><small>Find likely vocabulary and sentence gaps</small></span><Icon name="arrow" /></button><button className="disabled-action" disabled><Icon name="record" /><span><b>Record audio</b><small>Listening recognition · V2</small></span><em>SOON</em></button></div></section><section className="priority-practice listen-practice"><div><span className="section-kicker">TODAY’S LISTENING</span><h2>{dueCount} items selected for you</h2><p>Context meaning, recall, fill-in-the-blank, and new workplace situations.</p><div className="practice-tags"><span>Weak in speech</span><span>Due today</span><span>From your meetings</span></div></div><button onClick={() => navigate("today")}>Start 6-minute practice <Icon name="arrow" /></button></section><section className="skill-shortcuts"><button onClick={() => navigate("vocabulary")}><span className="shortcut-icon"><Icon name="book" /></span><span><b>Review my words</b><small>Mastery across recognition, meaning, recall, and use</small></span><em>{dueCount} due</em></button><button onClick={() => navigate("sentence")}><span className="shortcut-icon"><Icon name="listen" /></span><span><b>Understand sentences</b><small>Learn what the coworker meant—not only each word</small></span><em>2 ready</em></button></section><section className="meeting-insight"><div className="section-heading"><h2>Latest meeting intelligence</h2><button onClick={() => startImport("listening")}>Add new</button></div><div className="meeting-card"><span>AUG<br />18</span><div><b>Product Launch Readiness</b><p>5 expressions discovered · 2 meaning checks remain</p><div><em>workaround</em><em>pressure-test</em><em>socialize</em></div></div><button onClick={() => navigate("today")} aria-label="Review Product Launch Readiness">→</button></div></section></main>;
}

function SpeakingHome({ navigate, startImport, analyzed }: { navigate: (view: View) => void; startImport: (purpose: ImportPurpose) => void; analyzed: boolean }) {
  return <main className="app-page"><section className="tab-intro"><span className="eyebrow">EXPRESS YOURSELF CLEARLY</span><h1>Speak</h1><p>Bring in what you actually said. Your coach finds recurring communication patterns—not endless sentence corrections.</p></section><section className="capture-hub speak-hub"><span className="section-kicker">ADD YOUR SPEAKING</span><h2>Let your work show us the problem.</h2><div className="capture-row"><button onClick={() => startImport("speaking")}><Icon name="text" /><span><b>Paste meeting transcript</b><small>We’ll isolate turns labeled “You”</small></span><Icon name="arrow" /></button><button className="disabled-action" disabled><Icon name="record" /><span><b>Record my answer</b><small>Pronunciation and fluency · V2</small></span><em>SOON</em></button></div><p className="capability-line"><Icon name="check" /> Text reliably evaluates grammar, clarity, structure, vocabulary, and naturalness.</p></section><section className="priority-practice speak-practice"><div><span className="section-kicker">TOP FOCUS · HIGH IMPACT</span><h2>Lead with the recommendation</h2><p>Your listener waits too long to learn what you want. Detected in 3 recent examples.</p><div className="practice-tags"><span>Impact 5/5</span><span>Frequency 3×</span><span>Foundational</span></div></div><button onClick={() => navigate("speaking-practice")}>Start 10-minute practice <Icon name="arrow" /></button></section>{analyzed && <div className="analysis-ready"><Icon name="check" /><span><b>New transcript analyzed</b>Your recurring patterns and practice priority were updated.</span></div>}<section className="error-map-preview"><div className="section-heading"><h2>My English Error Map</h2><span>Last 30 days</span></div>{weeklyFocus.map((focus, index) => <article key={focus.title}><span className={index === 0 ? "high" : "medium"} /><div><b>{focus.title}</b><p>{focus.why}</p><small>{focus.evidence}</small></div><em>{index === 0 ? "High" : "Medium"}</em></article>)}</section><section className="intent-preview"><span className="section-kicker">INFORMATION LOSS</span><h2>What you meant ≠ what the listener heard</h2><p>Your latest example made it unclear whether the launch, announcement, or both should wait.</p><button onClick={() => navigate("speaking-practice")}>See the clearer version <Icon name="arrow" /></button></section></main>;
}
...
function TodayListening({ items, index, answer, setAnswer, revealed, setRevealed, complete, rate, restart, navigate }: { items: VocabularyItem[]; index: number; answer: string; setAnswer: (value: string) => void; revealed: boolean; setRevealed: (value: boolean) => void; complete: boolean; rate: (rating: "Hard" | "Almost" | "Got it") => void; restart: () => void; navigate: (view: View) => void }) {
  ...
  if (!item) return <main className="app-page detail-page practice-page"><PageHead title="Today’s Listening" subtitle="Nothing is due right now." onBack={() => navigate("listening")} /><section className="practice-card empty-state"><Icon name="spark" /><h2>You’re caught up.</h2><p>Add a real conversation to create new practice.</p><button className="solid-button" onClick={() => navigate("import")}>Add conversation</button></section></main>;
  if (complete) return <main className="app-page detail-page practice-page"><PageHead title="Session complete" subtitle="Today’s performance updated your review plan." onBack={() => navigate("listening")} /><section className="session-complete"><span>6 min</span><h2>You strengthened {Math.min(items.length, 3)} expressions.</h2><p>Next time, these will return in new workplace contexts.</p><div><b>Evidence captured</b><small>Contextual understanding + recall confidence</small></div><button className="solid-button" onClick={() => navigate("home")}>Done for now</button><button className="text-button" onClick={restart}>Practice again</button></section></main>;
