...
Quick Add now continues into an immediate study state instead of ending at the save receipt. The just-added card observes the account-backed vocabulary item while enrichment runs, then reveals simple meaning, optional Chinese support, phonetic spelling, browser pronunciation playback, practical usage guidance, collocations, and a workplace example. Re-adding an existing term opens the same teaching state rather than creating another record.

Study and retrieval are intentionally different surfaces. Word Library is searchable and always shows the answer so the learner can understand, look up, and review any saved item. Vocabulary Practice hides meaning and examples until the learner responds, then provides corrective feedback and updates the spaced-review schedule. Both surfaces read the same user-owned vocabulary record, so study content, attempts, and future review timing stay connected.
